...
docker rm rcca_awac_v3 2>/dev/null || true

docker run --name rcca_awac_v3 --gpus all --shm-size=24g --init -d \
  ...
  python3 /opt/eve_training/training_scripts/DualDeviceNav_train.py \
    ...
    --log_std_min -2 \
    --heuristic_seeding 10 \
    --heuristic_from_restore \
    --min_success_rate 1.0 \
    --max_seeding_multiplier 20 \
    --seeding_success clean_thread \
    --heuristic_factory "util.heuristic_policy_rcca:RCCAHeuristicActionFunctionFactory" \
    --save_heuristic_cache /opt/eve_training/results/rcca_heuristic_fork_v3.npz \
    --heatup_episodes 10 \
    --save_heatup_cache /opt/eve_training/results/rcca_heatup_fork_v3.npz \
    ...
    -nw 16 -d cuda:0
# Seed = heuristic-from-restore (fork) until 10 RCCA clean threads
# (--heuristic_seeding 10 --min_success_rate 1.0 --seeding_success clean_thread
# --heuristic_from_restore) + 10 random heatup episodes (--heatup_episodes 10),
# then --pretrain_updates 1000, then online AWAC. NO cache files.
