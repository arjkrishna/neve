#!/bin/bash
# Plan v12 Stage 2 — standalone multi-target random heatup harvest from z=345.
#
# Runs DualDeviceNav_train.py in --heatup_only --multi_target_heatup mode:
#   - Builds ONE intervention + ONE MultiTargetEnv5 wrapping 4 virtual envs
#     (RCCA primary + LCCA / RVA / LVA secondaries) on a shared SOFA backend.
#   - Each SOFA tick advances the physics once; every virtual env computes its
#     own (obs, reward, terminal) and emits a target-tagged Episode.
#   - runner.py heatup-save partitions Episodes by target_branch_idx and
#     writes 4 per-target .npz files with R17-safe is_clean metadata + the
#     downstream-filter fields (final_branch_short, target_branch_short,
#     received_correct_daughter, received_wrong_daughter) so the user can run
#     "success = is_clean AND received_correct_daughter" filters directly.
#   - --heatup_only exits AFTER the .npz files are saved — NO pretrain, NO
#     AWAC training, NO online explore.
#
# Output layout under /opt/eve_training/results inside container (= saved/ on
# host). ROLLING CHUNK files every --heatup_save_every SOFA episodes, per
# worker w / pid p / chunk c (memory-safe: each worker flushes + frees its
# batch, so RAM stays bounded over a multi-day run; analyzer globs+concatenates):
#   heatup_z345_multitarget/
#     Version A — by TARGET graded (mostly failures for that target):
#       heatup_targetpart_{RCCA,LCCA,RVA,LVA}_w{w}_p{p}_c{c}.npz
#     Version B — "snapshot version": wire physically ENDED in that daughter
#       (final_branch_short==target_branch_short, ALL reasons — success /
#        overshoot / WBT / WFS / unknown):
#       heatup_endedin_{RCCA,LCCA,RVA,LVA}_w{w}_p{p}_c{c}.npz
#
# Standing knob: --base_seed 42 (Plan v12 R16). Same seed reproduces the
# harvest; per-worker + per-episode RNGs are derived as
#   base_seed + worker_id * 1_000_003 + episode_idx
# guaranteeing disjoint streams across workers.
#
# To stop the container manually: `docker stop heatup_z345_v1`. SIGTERM is
# handled by the runner — partial buffers are immediately consumable.

set -e
export MSYS_NO_PATHCONV=1

docker rm heatup_z345_v1 2>/dev/null || true

docker run --name heatup_z345_v1 --gpus all --shm-size=24g --init -d \
  ...
  python3 /opt/eve_training/training_scripts/DualDeviceNav_train.py \
    ...
    --heatup_only \
    --base_seed 42 \
    --save_heatup_cache /opt/eve_training/results/heatup_z345_multitarget_run2/heatup.npz \
...
#   docker stop -t 60 heatup_z345_v1   # SIGTERM → workers finish current SOFA
#                                      # episode + flush final <500 batch, then
#                                      # exit. -t 60 gives the grace window so
#                                      # the in-flight batch is not SIGKILLed.
#   docker logs -f heatup_z345_v1      # live progress; EPISODE_OUTCOME lines
