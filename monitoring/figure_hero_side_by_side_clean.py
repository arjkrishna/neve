...
def style(ax, title):
    ...
    ax.text2D(0.40, 0.97, title, transform=ax.transAxes, fontsize=11,
              va="top", ha="left")
...
fig.suptitle(
    "Same generated vasculature, two representations — "
    "identical scale and view\n"
    "blue = on-path RCCA route,  green = RVA,  red = other branches,  "
    "orange = planned path,  X = target,  star = fixed insertion",
    fontsize=13)
fig.tight_layout(rect=[0, 0, 1, 0.90])
fig.savefig(os.path.join(OUT, "hero_centerline_vs_mesh_clean.png"), dpi=160)
plt.close(fig)
print("wrote hero_centerline_vs_mesh_clean.png", flush=True)
