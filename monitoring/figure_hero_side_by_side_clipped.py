...
Writes hero_centerline_vs_mesh_clipped.png — as the original, but all
geometry is clipped to the axis box so the aorta no longer runs off the
...
for g in ("other", "rva", "onpath"):
    ...
    for k, br in enumerate(groups[g]):
        ...
        if c.size == 0:
            continue
        first = True
        for seg in clipped_segments(c, LIMS):
            ax1.plot(seg[:, 0], seg[:, 1], seg[:, 2], color=color,
                     alpha=0.55 if g == "other" else 0.92,
                     linewidth=1.0 if g == "other" else 2.0,
                     label=(label if (k == 0 and first) else None))
            first = False
...
for g in ("other", "rva", "onpath"):
    ...
    v, f = meshes[g]
    color, label = COL[g]
    fc = clipped_mesh(v, f, LIMS)
    if fc.size:
        ax2.add_collection3d(Poly3DCollection(
            v[fc], facecolor=(*mcolors.to_rgb(color), MESH_ALPHA[g]),
...
for g in ("other", "rva", "onpath"):
    ...
    for br in groups[g]:
        ...
        if c.size == 0:
            continue
        for seg in clipped_segments(c, LIMS):
            ax2.plot(seg[:, 0], seg[:, 1], seg[:, 2], color=color,
...
fig.savefig(os.path.join(OUT, "hero_centerline_vs_mesh_clipped.png"), dpi=160)
plt.close(fig)
print("wrote hero_centerline_vs_mesh_clipped.png", flush=True)
