...
TIGHT variant: panels 1-2 are framed per-axis (like panel 3) instead of on the
equal 122 mm cube, and only ONE legend is drawn. The cube gave x and y the
RCCA's ~245 mm z-extent, so roughly a third of each panel was empty margin;
equal scale is preserved by handing the data ranges to set_box_aspect.

Writes three_panel_overview_tight.png (new file; overwrites nothing).
...
# ---- layout, in INCHES ---------------------------------------------------
FIG_W, FIG_H = 12.9, 9.4
PANEL_H = 7.55                 # inscribed square of every panel
PANEL_Y = 0.70
SQ = [7.55, 7.55, 7.55]        # rect widths (>= PANEL_H so height binds)
# Content half-widths measured off the TIGHT render: ~2.1 in for the two
# framed panels (box + tick labels), ~1.3 in for the devices slab. Spaced so
# the boxes clear each other by ~0.4 in and nothing overlaps.
CENTER_X = [2.35, 6.75, 10.55]
TITLE_Y = 0.985
LEGEND_Y = 0.930
...
LIMS = [(LO[i], HI[i]) for i in range(3)]
BOX = tuple(HI - LO)
print("  shared frame extent mm  x %.0f  y %.0f  z %.0f" % BOX, flush=True)


def clip_line(c):
    """NaN out polyline points outside the frame so lines stop at the box.

    matplotlib's 3-D axes do not clip artists to the axis limits: with the
    old equal cube the non-navigated branches happened to fall inside, but
    under tight per-axis limits they were drawn sprawling over the white
    space next to the panel. NaN breaks the line instead of clamping it, so
    no spurious segment is drawn across the box.
    """
    c = np.asarray(c, dtype=float).copy()
    if c.size == 0:
        return c
    outside = ((c < LO - 1e-9) | (c > HI + 1e-9)).any(axis=1)
    c[outside] = np.nan
    return c


def clip_faces(v, f):
    """Drop mesh triangles with any vertex outside the frame."""
    if f.size == 0:
        return f
    inside = ((v >= LO - 1e-9) & (v <= HI + 1e-9)).all(axis=1)
    return f[inside[f].all(axis=1)]
...
def add_extras(ax, legend):
    if pp is not None and len(pp) > 1:
        q = clip_line(pp)
        ax.plot(q[:, 0], q[:, 1], q[:, 2], color="#ffaa00", linewidth=2.8,
...
def style(ax, cube=True):
    if cube:
        ...
        try:
            ax.set_box_aspect(BOX)      # equal scale, no dead x/y width
        except Exception:
            pass
    ax.view_init(elev=ELEV, azim=AZIM)
    ax.set_xlabel("x (mm)", fontsize=9, labelpad=0)
    ax.set_ylabel("y (mm)", fontsize=9, labelpad=0)
    ax.set_zlabel("z (mm)", fontsize=9, labelpad=0)
    ax.tick_params(labelsize=7)


def place_legend(ax, i, ncol=1):
    ...
    fig.legend(h, l, loc="upper center", ncol=ncol, fontsize=8,
               bbox_to_anchor=(CENTER_X[i] / FIG_W, LEGEND_Y))
...
add_extras(ax2, legend=False)
style(ax2)
# No legend on the mesh panel: every entry except "centerlines (overlaid)"
# duplicated the Left panel's, and that one is stated in the caption instead.
...
ax3.set_xticklabels([])
ax3.set_yticklabels([])
ax3.set_xlabel("x (mm)", fontsize=9, labelpad=-16)
ax3.set_ylabel("y (mm)", fontsize=9, labelpad=-16)
ax3.set_zlabel("z (mm)", fontsize=9, labelpad=8)
place_legend(ax3, 2, ncol=1)
print("  devices extent mm  x %.0f  y %.0f  z %.0f" % tuple(hi - lo), flush=True)

TITLES = ["Centerlines\n(what the pathfinder and observations use)",
          "Vessel mesh + centerlines\n(what SOFA simulates and collides against)",
...
fig.savefig(os.path.join(OUT, "three_panel_overview_tight.png"), dpi=150)
plt.close(fig)
print("wrote three_panel_overview_tight.png", flush=True)
