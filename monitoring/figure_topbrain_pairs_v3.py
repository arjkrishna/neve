#!/usr/bin/env python3
"""Visual QC of the grafted TopBrain anatomies, two per image -- v3 meshes.

Draws the BAKED collision mesh (vessel_architecture_collision.obj) of each
anatomy, which in v3 carries the patient's real ICA surface unioned into the
tube host, so the siphon is shown as segmented, bulges and all. The one mesh
is split into the three colour groups by nearest centerline branch, so the
legend and framing match the earlier figures. New folder; nothing overwritten.
...
_ap.add_argument("root", nargs="?", default="/opt/eve_training/topbrain_data/anatomies_v3")
_ap.add_argument("out", nargs="?", default="/opt/eve_training/saved/figs/topbrain_v3")
...
def split_obj(root_dir, g, target_faces=9000):
    """The baked v3 mesh, cut into the three groups by nearest branch."""
    import pyvista as pv
    from scipy.spatial import cKDTree
    m = pv.read(os.path.join(root_dir, "vessel_architecture_collision.obj")).extract_surface().triangulate()
    if m.n_cells > target_faces:
        m = m.decimate(1.0 - target_faces / float(m.n_cells), volume_preservation=True)
    v = np.asarray(m.points, dtype=np.float32)
    ff = np.asarray(m.faces, dtype=np.int64).reshape(-1, 4)[:, 1:]
    cen = v[ff].mean(axis=1)
    keys, pts = [], []
    for k in ("onpath", "rva", "other"):
        for b in g[k]:
            c = np.asarray(b.coordinates, float)
            keys += [k] * len(c); pts.append(c)
    d, j = cKDTree(np.concatenate(pts)).query(cen)
    keys = np.asarray(keys)
    return {k: (v, ff[keys[j] == k]) for k in ("onpath", "rva", "other")}
...
def draw(ax, case, legend):
    meshes = split_obj(os.path.join(_args.root, case["stem"]), case["g"])
