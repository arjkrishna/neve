#!/usr/bin/env python3
"""Visual QC of the grafted TopBrain anatomies, two per image -- v2 meshes.

Identical to figure_topbrain_pairs.py except that the per-group meshes
(on-path route, RVA, everything else) are built with the v2 signed-distance
mesher (topbrain_tools/sdf_mesher.py) instead of the voxel/Gaussian one, so
what is drawn is the surface the v2 anatomies actually load into SOFA:
thin distal siphons at their declared calibre rather than eroded to a thread.
Output goes to a NEW folder; nothing in saved/figs/topbrain is touched.
...
from sdf_mesher import tube_field, iso_surface, decimate_to

_ap = argparse.ArgumentParser()
_ap.add_argument("root", nargs="?", default="/opt/eve_training/topbrain_data/anatomies")
_ap.add_argument("out", nargs="?", default="/opt/eve_training/saved/figs/topbrain_v2")
...
FIG_SPACING = 0.6     # figures only: visually identical to 0.45 at a third of the cost


def mesh_of(branches, tag, target_faces=9000):
    """v2 mesh of one branch group: SDF at FIG_SPACING, iso 0, quadric to target."""
    m = decimate_to(iso_surface(tube_field(list(branches), spacing=FIG_SPACING)), target_faces)
    v = np.asarray(m.points, dtype=np.float32)
