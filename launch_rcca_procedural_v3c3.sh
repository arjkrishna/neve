#!/bin/bash
# RL_IMPROV_16 (v3c3) — rcca_procedural_v3c3: v3c2 + the two CRITIC-STABILITY levers
# from the verified collapse forensic (2026-07-24). Reward pair UNCHANGED;
# same seed (saved/rcca_proc_heatup_v3c/seed.npz — stamps match).
#
# THE FORENSIC (self-verified against CSV + chunks + step logs):
#   v3c2's decline was NOT the uncapped slack tail (code-bounded 3.28/step;
#   capped run1's q1 trajectory was IDENTICAL through overlap; the -15.4
#   outliers are a single SOFA blow-up episode, tip3d km outside anatomy,
#   AFTER divergence onset). It was ENDOGENOUS TD/bootstrap instability:
#   contact-state mean-rail (17.6% at peak -> 31%) -> policy's sampled
#   next-actions go OOD -> min-double-Q extrapolates negatively -> plain
#   Bellman target follows (q1 -1.9@explore376k, -4@493k, -90 by end) ->
#   advantage ranking degrades -> worse cloning -> more rail.
#
# THE TWO LEVERS (both trainer-side; reward/obs/terminals untouched):
#   --contact_mean_penalty 0.05 : STEERING — contact-gated anti-rail keeps
#       the contact-state mean off the rail so next-actions stay
#       in-distribution (attacks the root of the bootstrap divergence).
#       Free states keep --action_mean_penalty 0.005.
#   --q_target_floor -10.0 : AIRBAG — clamps the Bellman target below any
#       honest return (measured min ~-7, bulk p1 -2.9) so the self-feeding
#       negative spiral is bounded regardless of mechanism.
#   --awac_mode_adv_norm stays OFF (unsafe until q1 confirmed stable).
#
# GATES: q1_mean (CSV col12) must hold > -2.5 and FLAT through u~300k;
#   clamp_fraction (col20) < 10%; eval1 vs v3c2's 58.2% and v3c run1's
#   62.2%; eval2 >= eval1 - noise (the point both prior runs never held).
# ABORT: q1 < -3 and still descending at u~200k; clamp > 20% sustained;
#   eval reward negative at eval2.
set -e
...
docker rm rcca_procedural_v3c3 2>/dev/null || true

docker run --name rcca_procedural_v3c3 --gpus all --shm-size=24g --init -d \
  ...
  -e STUCK_CHECKPOINT_DIR=/opt/eve_training/results/rcca_v3c3_stuck \
...
# Monitor:  docker logs -f rcca_procedural_v3c3   |   Stop:  docker stop rcca_procedural_v3c3
