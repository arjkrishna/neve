#!/bin/bash
# Plan v13 — RCCA AWAC obs-v3: obs-v3 (PathLookahead3D) + recovery horizon + v2 stability stack.
#
...
#   3. Seed: rcca_awac_seed_obsv3.npz (96-dim, from heatup_z345_obsv3 via
...
docker rm rcca_awac_obsv3 2>/dev/null || true

docker run --name rcca_awac_obsv3 --gpus all --shm-size=24g --init -d \
  ...
  python3 /opt/eve_training/training_scripts/DualDeviceNav_train.py \
    ...
    --heatup_cache_file /opt/eve_training/results/rcca_awac_seed_obsv3.npz \
...
# Monitor:  docker logs -f rcca_awac_obsv3   |   Stop:  docker stop rcca_awac_obsv3
