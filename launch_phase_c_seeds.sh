#!/bin/bash
# Phase C paired comparison: 500 fresh seeds, each run with all 9 variants.
# Total = 500 × 9 = 4500 episodes. Snapshots bucketed by seed so all 9
# variants for the same target are side-by-side in snapshots/RVA/seed_<S>/.
set -e
export MSYS_NO_PATHCONV=1

docker rm env5_rl8_phase_c_paired_500 2>/dev/null || true

docker run --name env5_rl8_phase_c_paired_500 --gpus all --shm-size=24g --init -d \
  ...
  -v "D:\neve\.claude\worktrees\rl_improv_8\training _scripts\run_phase_c_grid.py:/opt/eve_training/training_scripts/run_phase_c_grid.py" \
  -v "D:\neve\.claude\worktrees\rl_improv_8\training _scripts\run_phase_c_seeds.py:/opt/eve_training/training_scripts/run_phase_c_seeds.py" \
