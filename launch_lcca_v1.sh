#!/bin/bash
# Plan v4 — LCCA-target heuristic v1 (50 episodes, smoke test).
# Uses LCCAHeuristicActionFunctionFactory: Phase A fixed -z, Phase B
# dynamic-tangent at s+5 with original sign (LCCA-aimed tangent),
# Phase C dynamic-tangent at s+5. Decision rule: if LCCA-reach >= 80%,
# ship v1; if < 80%, build v2 (RVA-tangent + flipped sign per plan).
set -e
export MSYS_NO_PATHCONV=1

docker rm env5_rl8_LCCA_v1_500ep 2>/dev/null || true

docker run --name env5_rl8_LCCA_v1_500ep --gpus all --shm-size=24g --init -d \
  ...
  eve-training-fixed \
  python3 /opt/eve_training/training_scripts/heuristic_run_LCCA.py \
    --env_version 5 \
    -n env5_rl8_LCCA_v1_500ep \
    --insertion_z 345 \
    --episodes 500 \
    --snapshots mesh \
    -nw 16 -d cuda:0
