#!/bin/bash
# RCCA-target heuristic v1 — exact mirror of RVA's Phase A/B/C with
# daughter junction token swapped to RCCA. 500 episodes for parity
# with the RVA v2cfg-500-inst run.
set -e
export MSYS_NO_PATHCONV=1

docker rm env5_rl8_RCCA_v1_500ep 2>/dev/null || true

docker run --name env5_rl8_RCCA_v1_500ep --gpus all --shm-size=24g --init -d \
  ...
  eve-training-fixed \
  python3 /opt/eve_training/training_scripts/heuristic_run_RCCA.py \
    --env_version 5 \
    -n env5_rl8_RCCA_v1_500ep \
    --insertion_z 345 \
    --episodes 500 \
    --snapshots mesh \
    -nw 16 -d cuda:0
