...
docker rm rcca_sac_v1 2>/dev/null || true

docker run --name rcca_sac_v1 --gpus all --shm-size=24g --init -d \
  ...
  python3 /opt/eve_training/training_scripts/DualDeviceNav_train.py \
    ...
    --save_heatup_cache /opt/eve_training/results/rcca_heatup_cache.npz \
    -nw 16 -d cuda:0
