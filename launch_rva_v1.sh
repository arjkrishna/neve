#!/bin/bash
# RVA-target heuristic v1 — exact mirror of RVA's Phase A/B/C with
# daughter junction token swapped to RVA. 500 episodes for parity
# with the RVA v2cfg-500-inst run.
set -e
export MSYS_NO_PATHCONV=1

docker rm env5_rl8_RVA_v1_500ep 2>/dev/null || true

docker run --name env5_rl8_RVA_v1_500ep --gpus all --shm-size=24g --init -d \
