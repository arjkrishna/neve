#!/bin/bash
# GENERIC replay WITH per-step trajectory RECORDING (ANIM_PIPELINE tier 2), host or procedural.
# = launch_eval_anatomies.sh + util/traj_record.py mount + rl_improv_16_resume/saved mounted at
#   /opt/eve_training/results16 (procedural checkpoints) + SEED_LIST + --traj_record.
#   EXTRA_FLAGS defaults to --real_patient_anatomy (HOST); EXTRA_FLAGS='' evaluates on the
#   procedural varied anatomies (per-worker seed streams).
# Masked checkpoints: EVE_RL_ACTOR_ZERO_OBS=... [EVE_RL_CRITIC_ZERO_OBS=...] ./launch_eval_traj.sh <ckpt>
...
EXTRA_FLAGS="${EXTRA_FLAGS---real_patient_anatomy}"
# SEED_LIST=validation -> the trainer's 98 EVAL_SEEDS; SEED_LIST=<comma list>; unset -> seed_base band
SEED_LIST="${SEED_LIST:-}"
...
_REC="--traj_record"
[ -n "$SEED_LIST" ] && _REC="$_REC --seed_list $SEED_LIST"
NAME="${NAME:-eval_traj}"
...
docker run --name "$NAME" --rm -i --gpus all --shm-size=16g \
  ...
  python3 /opt/eve_training/training_scripts/eval_anatomies.py \
    ...
    --max_steps "$MAX_STEPS" \
    $ARCH_FLAGS $ENV_FLAGS $EXTRA_FLAGS $_REC
#
