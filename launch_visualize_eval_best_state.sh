#!/bin/bash
# Visualize the eval-only pid19116 run in SOFA via X11 forwarding to VcXsrv.
#
# Requirements:
#   - VcXsrv running on the Windows host (with "Disable access control" checked)
#   - host.docker.internal resolves to the host (Docker Desktop default)
#
# Mounts mirror launch_rcca_eval_best_state.sh + the new visualize script.
...
docker rm rcca_visualize_pid19116 2>/dev/null || true
