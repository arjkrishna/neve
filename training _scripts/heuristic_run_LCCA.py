"""Heuristic-only run targeting LCCA daughter ONLY.

One of four per-daughter runners (LCCA / LVA / RCCA / RVA) used to
...
Differences from the generic ``heuristic_only_run.py``:
  - ``TARGET_BRANCHES`` is a single-element list (LCCA only).
...
Example (from inside the SOFA container):
    python3 /opt/eve_training/training_scripts/heuristic_run_LCCA.py \\
        --env_version 5 -n env5_rl8_LCCA_50ep \\
