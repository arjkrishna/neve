...
TOPBRAIN_ONLY="${TOPBRAIN_ONLY:-}"
_TB="--topbrain"
[ -n "$TOPBRAIN_ONLY" ] && _TB="$_TB --topbrain_only $TOPBRAIN_ONLY"
EXTRA_FLAGS="${EXTRA_FLAGS:-$_TB --cath_slack_coef 0.5 --progress_tip_mode avg --avg_gw_weight 0.5}"
...
docker run --name "$NAME" --rm -i --gpus all --shm-size=16g \
  ...
  -v "D:\Arjun\workspace\neve\topbrain_data:/opt/eve_training/results_topbrain:ro" \
  eve-training-fixed \
