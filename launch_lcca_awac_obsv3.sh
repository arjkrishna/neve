#!/bin/bash
# Plan v13 — LCCA AWAC obs-v3: obs-v3 (PathLookahead3D) + recovery horizon + v2 stability stack.
#
...
docker rm lcca_awac_obsv3 2>/dev/null || true

docker run --name lcca_awac_obsv3 --gpus all --shm-size=24g --init -d \
...
# Monitor:  docker logs -f lcca_awac_obsv3   |   Stop:  docker stop lcca_awac_obsv3
