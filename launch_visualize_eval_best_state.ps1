# PowerShell version of launch_visualize_eval_best_state.sh
# Run from PowerShell with:
#     .\launch_visualize_eval_best_state.ps1
# (you may need to enable script execution first:)
#     Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
#
# Requirements:
#   - VcXsrv running on the Windows host (with "Disable access control" checked)

$RunDir   = "/opt/eve_training/results/eve_paper/neurovascular/full/mesh_ben/2026-05-25_031951_rcca_awac_v3"
$EvalCkpt = "$RunDir/checkpoints/checkpoint250531.everl"

docker rm -f rcca_visualize_pid19116 2>$null

docker run --name rcca_visualize_pid19116 --gpus all --shm-size=24g --init --rm `
  -e DISPLAY=10.113.123.221:0 `
  -v "D:\neve\.claude\worktrees\rl_improv_8\training _scripts\visualize_eval_best_state.py:/opt/eve_training/training_scripts/visualize_eval_best_state.py" `
  -v "D:\neve\.claude\worktrees\rl_improv_8\training _scripts\eval_policy_from_state.py:/opt/eve_training/training_scripts/eval_policy_from_state.py" `
