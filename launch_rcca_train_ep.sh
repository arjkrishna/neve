#!/bin/bash
# Episode-RL variant (experiment #1 in the SAC-variant comparison).
# Same env5 / localguidance / reward / heuristic-seed pipeline as the step
# run — only the replay buffer + network differ:
#   - Episode RL: VanillaEpisodeShared (whole-episode buffer), LSTM embedder
#   - replay_mode=episode (default), hidden=[900,900,900,900], embedder 1x500
#   - NO PER, episode-default LR / batch / update-per-explore (1/20)
# Loads the saved heuristic cache (100 ep) and a 20-episode heatup subset
# (rcca_heatup_cache_20.npz: 3 terminal-successes + 17 random) — skips both
# seeding and heatup. obs/reward are unchanged so the caches are compatible.
set -e
export MSYS_NO_PATHCONV=1

docker rm rcca_sac_episode_v1 2>/dev/null || true

docker run --name rcca_sac_episode_v1 --gpus all --shm-size=24g --init -d \
