...
class Runner(EveRLObject):
    ...
    def _maybe_save_policy_snapshot(self):
        ...
        if current_step >= self._next_snapshot_step:
            try:
                snapshots_dir = os.path.join(self.diagnostics_folder, "policy_snapshots")
                os.makedirs(snapshots_dir, exist_ok=True)
                snapshot_path = os.path.join(snapshots_dir, f"policy_{current_step}.pt")
                
                if hasattr(self.agent, 'save_policy_snapshot'):
                    self.agent.save_policy_snapshot(snapshot_path)
                    self.logger.info(f"Saved policy snapshot at step {current_step}")
                ...
                self.logger.warning(f"Failed to save policy snapshot: {e}")
