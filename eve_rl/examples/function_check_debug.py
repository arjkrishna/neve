...
def sac_training(
    ...
    with open(logfile, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow([lr, gamma, hidden_layers])
        writer.writerow(["Episodes", "Steps", "Reward", "Success", "Memory_MB"])
    ...
    try:
        while step_counter.exploration < training_steps:
            print(f"DEBUG: Step {step_counter.exploration}, before explore")
            agent.explore(steps=consecutive_explore_steps)
            step_counter = agent.step_counter
            update_steps = step_counter.exploration - step_counter.update
            print(f"DEBUG: Step {step_counter.exploration}, before update")
            agent.update(steps=update_steps)
            print(f"DEBUG: Step {step_counter.exploration}, after update")

            if step_counter.exploration >= next_eval_step_limt:
                # Get memory usage
                current, peak = tracemalloc.get_traced_memory()
                memory_mb = current / 10**6
                print(f"DEBUG: Memory usage: {memory_mb:.2f} MB (peak: {peak / 10**6:.2f} MB)")
                ...
                print(f"DEBUG: Checkpoint saved, before evaluate")
                ...
                print(
                    f"Steps: {step_counter.exploration}, Reward: {reward}, Success: {success}, Memory: {memory_mb:.2f} MB"
                )
        ...
        print(f"ERROR: Python exception occurred: {e}")
        import traceback
        traceback.print_exc()
        raise
