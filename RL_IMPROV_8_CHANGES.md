...
## 28. Phase C Variant Catalog + Factorial Grid Test (Plan v3)

### Context

§27's Phase C is one design out of a large option space. Rather than test each variant as a separate 500-episode experiment (~16 h), build a polymorphic factory that dispatches one of 9 variants at runtime per episode, then run a factorial schedule (states × strategies) in a single docker job.

### Fix

**Files:**
- [training _scripts/util/heuristic_policy_rva.py](training _scripts/util/heuristic_policy_rva.py) — extended with `PHASE_C_VARIANTS` dict (C0..C8), `_phase_c_target()` helper, stagnation tracking for C8
- [training _scripts/util/env5.py](training _scripts/util/env5.py) — captures `_phase_c_variant` from `options['phase_c_variant']` in `reset()`; logs `phase_c_variant=` in EPISODE_START
- [training _scripts/util/checkpoint_restore.py](training _scripts/util/checkpoint_restore.py) — `_pick_index` extended with `explicit_id` (= `options['checkpoint_id']`) for factorial state-restore
- [training _scripts/run_phase_c_grid.py](training _scripts/run_phase_c_grid.py) — **New.** Factorial schedule runner (checkpoints × variants), uses CheckpointRestoreWrapper to skip the upstream traversal each iteration
- [training _scripts/run_phase_c_seeds.py](training _scripts/run_phase_c_seeds.py) — **New.** Paired-comparison runner: N fresh seeds × variants, no SOFA restore (full femoral→target traversal each episode)
- [analyze_phase_c_grid.py](analyze_phase_c_grid.py) — **New.** Per-variant summary + heatmap CSV + per-checkpoint winner ranking

### Variant catalog

| ID | trigger_before | cavity_gw_trans | daughter_gw_trans | target_kind | lookahead | stagnation |
|---|---|---|---|---|---|---|
| C0 | None | — | — | — | — | — (no Phase C) |
| C1 | 5 | 3 | 2 | fixed_y | — | — |
| C2 | 5 | 3 | 2 | dynamic_tangent | 5 | — (= §27 production) |
| C3 | 5 | 3 | 2 | dynamic_tangent | 1 | — |
| C4 | 5 | 3 | 2 | dynamic_tangent | 10 | — |
| C5 | 5 | 3 | 2 | position_to_target | — | — |
| C6 | 5 | 2 | 2 | dynamic_tangent | 5 | — |
| C7 | 15 | 3 | 2 | dynamic_tangent | 5 | — |
| C8 | 5 | 3 | 2 | dynamic_tangent | 5 | retract |

### Strategy 1 attempted (SOFA checkpoint restore) — FAILED

Captured 173 `.npz` checkpoints from v2cfg-500-inst at the moment wires' projection arclength crossed `rva_jn_arc`. Smoke test (5 cps × 9 variants = 45 episodes) revealed **36% of restored states placed the wire in wrong locations** (RCCA wedge, LCCA-jn floor, internal LCCA junction) because the trigger fires on projection-arclength, which leaps 30-50 mm in single SOFA steps at the bif2 cavity. Even "clean" checkpoints duplicated across workers due to RNG-pid hash collision. **Strategy abandoned.**

### Strategy 2 — paired-comparison full traversal (500 seeds × 9 variants = 4500 episodes)

Schedule generated by `build_paired_schedule()`: each of N fresh seeds × every variant, schedule grouped by seed (un-shuffled) so each `seed_<S>/` snapshot folder fills with 9 PNGs (one per variant) within a small wall-clock window. Snapshot path bucketing also changed in [snapshot.py](training _scripts/util/snapshot.py) to `<target>/seed_<seed>/<variant>_<reason>_step<S>_R<R>.png` (later reverted to legacy `<reason>/` layout per user preference).

### Result

| metric | value |
|---|---|
| Run | `env5_rl8_phase_c_paired_500` (4500 episodes, all 9 variants) |
| Successes | 0 across all variants on a "harder" RNG slice |
| Top arc on RVA | 26.4 mm (= ceiling matching v2cfg-500-inst single-variant runs) |
| C8 stagnation-retract | actively counterproductive — ejects wires from RVA into bridge(11), they can't re-thread |

### File index entries

| File | Change |
|---|---|
| `training _scripts/util/heuristic_policy_rva.py` | `PHASE_C_VARIANTS` dict; `_phase_c_target()` helper for `fixed_y` / `dynamic_tangent` / `position_to_target`; stagnation tracking (C8); `_detect_phase()` takes `variant_cfg` parameter |
| `training _scripts/util/env5.py` | `_phase_c_variant` capture in `reset()`; `phase_c_variant=` in EPISODE_START log |
| `training _scripts/util/checkpoint_restore.py` | `_pick_index(seed, explicit_id=None)` honors `options['checkpoint_id']` |
| `training _scripts/run_phase_c_grid.py` | **New** — factorial state×strategy grid runner |
| `training _scripts/run_phase_c_seeds.py` | **New** — paired (seed × variant) grid runner |
| `analyze_phase_c_grid.py` | **New** — per-variant summary + heatmap |
| `launch_phase_c_smoke.sh` / `launch_phase_c_seeds.sh` | **New** — docker launch scripts |

---

## 29. Reward Grace + Wrong-Branch-Timeout Suppression (env5)

### Context

Two systemic env5 issues emerged when investigating LCCA wedge episodes:

**Issue A (reward inflation from flicker):** At bif2 cavity, the wire's tip can be stationary while `cur_branch` flickers between `LVA.mrk` / `(11).mrk` / etc. every other SOFA step because the closest-centerline argmin is unstable in the trifurcation region. Every `on_path=1 → on_path=0` flip triggered `WRONG_BRANCH_ENTRY_PENALTY = -1.0`. Episodes physically stationary at (54, 43, 414) accumulated **-110 reward purely from flicker**, drowning out any signal about actual wire behavior.

**Issue B (wrong-branch-timeout firing on advancing wires):** In LVA-target episodes, the wire genuinely advances along LVA's path through the `(19)` segment (which is the deep extension of LVA in this dataset). `cur_branch` classifies as `(19).mrk`, `is_on_correct_path()` returns False because (19) isn't in `path_branch_set`, `_off_branch_steps` climbs to 50, env5 fires `wrong_branch_timeout` and truncates — even though the wire was advancing 0.3 mm/step into deeper LVA at z=440-520, never deflecting.

### Fix

**File:** [training _scripts/util/env5.py](training _scripts/util/env5.py) (around line 528-560)
...
if not terminated and not truncated:
    if not on_correct_path:
        ...
        # Per-daughter policies can set _heur_suppress_wrong_branch to True
        # when intentionally driving the wire off-path inside a daughter
        # region. When suppressed: NO penalty, NO truncation.
        suppress = bool(getattr(self, "_heur_suppress_wrong_branch", False))
        if not suppress:
            # 3-step grace before applying penalties: cur_branch can flicker
            # at junction cavities. Steps 1-2 no penalty; step 3 entry penalty
            # (once); step 4+ per-step penalty; timeout at 50.
...
Also `_resolve_termination_reason()` updated to honor the suppression flag when categorizing snapshot subdirs (otherwise episodes ran the full 600 steps but got filed under `wrong_branch_timeout/` cosmetically).

### Effect

- Reward composition for cavity-wedged LCCA episodes: was R≈−115 dominated by flicker; now R≈−2 to −7 (just per-step base penalties), accurately reflecting "wire reached LCCA daughter, parked stably"
- LVA episodes advancing through `(19)` segment now run to step 600 instead of being killed at step 50; rewards no longer get the −60 from per-step penalties either
- All non-suppressed episodes behave as before; the patch is a pure addition

### Per-daughter policies signal suppression via `base_env._heur_suppress_wrong_branch = True/False` each step. Default is False (= legacy behavior).

---

## 30. Per-Daughter Heuristic Policies — LCCA, RCCA, LVA

### Context

§22 created an RVA-target heuristic with arclength-windowed Phase A/B/C. The other three daughters (LCCA, LVA, RCCA) were still using the generic centerline-follower, which doesn't perform daughter-specific J-tip steering at the bif2 junction. Built per-daughter policies mirroring RVA's structure with daughter-specific path tokens and phase logic.

### Fix

Three new policy files, all wired into their respective per-daughter runners ([heuristic_run_LCCA.py](training _scripts/heuristic_run_LCCA.py), `_RCCA.py`, `_LVA.py`) via `_build_heuristic_factory()`.

#### A. [training _scripts/util/heuristic_policy_rcca.py](training _scripts/util/heuristic_policy_rcca.py) — **New** (exact RVA mirror)

Path topology: `trunk(2) → (0) → (11) bridge → RCCA daughter` (identical to RVA up to the daughter junction). Only difference from RVA's policy:
- Junction token: `_find_junction_arc(junctions, "(11)", "RVA")` → `_find_junction_arc(junctions, "(11)", "RCCA")`
- Class names: `RVAHeuristic*` → `RCCAHeuristic*`
- Phase string labels: `rva_*` → `rcca_*`
- Internal field: `_rva_jn_arc` → `_rcca_jn_arc`
- Diagnostic fields `_heur_rcca_*` AND `_heur_rva_phase` (= mirror so env5's STEP log picks up the phase regardless of which factory is in use)
- Phase A/B/C constants + variant catalog (C0-C8) unchanged

#### B. [training _scripts/util/heuristic_policy_lcca.py](training _scripts/util/heuristic_policy_lcca.py) — **New**

LCCA path topology turned out to be **simpler than expected**: `trunk(2) → (0) → LCCA` with only TWO junctions (no intermediate bridge like (11) for RVA/RCCA). Initial attempt anchored Phase B at a hypothetical `(0)→(19)` bridge that doesn't exist on the LCCA path — Phase B never fired.

Final design (after extensive iteration):
- Single junction cached: `_lcca_jn_arc = _find_junction_arc(junctions, "(0)", "LCCA")`
- Phase A: trunk-top, fixed `−z`, gw_trans=10 (RVA-mirror, unchanged)
- **Phase B: position-vector target to LCCA waypoint at `lcca_jn + 25 mm`** — adaptive: closed-loop computes unit vector from current tip to a fixed deep LCCA polyline point. Adapts to wire's position as it moves through the bif2 cavity. window `[lcca_jn − 15, lcca_jn + 5]`, gw_trans=1.5
- **Fix #1 applied: Phase B checked BEFORE Phase A in `_detect_phase`** — in window-overlap region Phase B wins; gives the slow deliberate position-vector steering a chance to fire instead of Phase A's fast `−z` push zipping the wire through the cavity
- Phase C: dynamic_tangent at s+5 (RVA-style) for in-daughter advance; window `s > lcca_jn + 5`, gw_trans=2.0

#### C. [training _scripts/util/heuristic_policy_lva.py](training _scripts/util/heuristic_policy_lva.py) — **New**

LVA path topology: `trunk(2) → (18) bridge → LVA daughter`. Wire's natural ascent up the trunk delivers it into (18) at the trunk-top crossing — same "deflection into (18)" failure mode observed in 10-30% of RVA/RCCA/LCCA runs, used INTENTIONALLY here. Two junctions total: `(2)→(18)` (not detected; default heuristic handles it) and `(18)→LVA` (Phase C anchor).

Final design:
- Single junction cached: `_lva_jn_arc = _find_junction_arc(junctions, "(18)", "LVA")`
- Phase A: window `[lva_jn − 30, lva_jn]` (= last 30 mm before LVA daughter entry, covers trunk-top + (18) bridge), target `+z` (degenerate vs ascending wire, just modulates gw_trans=10)
- Phase B: **doesn't exist** (no intermediate junction to thread)
- Phase C: `s > lva_jn` only (daughter sub-regime only, no cavity sub-regime), dynamic_tangent at s+5, gw_trans=2.0
- **LVA-specific override**: when `s > lva_jn AND is_on_correct_path() == False`, still fire Phase C `lva_daughter` (instead of falling back to default's retract). Wire stays in Phase C with dynamic_tangent target; body-shape physics decides motion. Sets `base_env._heur_suppress_wrong_branch = True` so env5 doesn't penalize or truncate.
  - Reason: `(19).mrk` is the deep extension of LVA in this dataset (same physical vessel, two centerline annotations). `is_on_correct_path()` returns False when projection lands in `(19)`'s polyline section. Without this override, wire-genuinely-advancing-into-LVA gets repeatedly retracted by the default off-path handler.

### Results

| daughter | success rate (term=True) | reach % | depth ceiling |
|---|---|---|---|
| RVA (v2cfg-500-inst, baseline) | 2/484 = 0.4% | 33% | arc=26 mm |
| RCCA (v1, 500 ep) | 15/484 = 3.1% | 33.7% | arc=43 mm |
| LCCA (v1 best — Fix #1 only, 50 ep) | 2/50 = 4% | 68% | arc=43 mm |
| LVA (v2 with overrides, 500 ep) | 95/484 = 19.6% | 27% | arc=137 mm |

LVA dramatically outperformed the others — the favorable geometry (natural ascent into (18)) + the off-path suppression override let many wires advance deep enough to hit shallow-to-mid targets. RCCA outperformed RVA on identical code purely due to the −x-dominant first-segment direction being more reachable than RVA's +y-dominant.

### File index entries

| File | Change |
|---|---|
| `training _scripts/util/heuristic_policy_rcca.py` | **New** — RVA mirror with `(11)→RCCA` token swap |
| `training _scripts/util/heuristic_policy_lcca.py` | **New** — LCCA policy; single junction `(0)→LCCA`; Phase B = position-vector to LCCA waypoint at lcca_jn+25 mm; Phase B priority over Phase A |
| `training _scripts/util/heuristic_policy_lva.py` | **New** — LVA policy; single junction `(18)→LVA`; Phase A = `+z` degenerate; Phase C only fires past lva_jn; off-path override inside daughter region sets `_heur_suppress_wrong_branch` |
| `training _scripts/heuristic_run_RCCA.py` (modify) | Slot in `RCCAHeuristicActionFunctionFactory` |
| `training _scripts/heuristic_run_LCCA.py` (modify) | Slot in `LCCAHeuristicActionFunctionFactory` |
| `training _scripts/heuristic_run_LVA.py` (modify) | Slot in `LVAHeuristicActionFunctionFactory` |
| `launch_rcca_v1.sh`, `launch_lcca_v1.sh`, `launch_lva_v1.sh` | **New** docker launch scripts (mirror of RVA launcher) |
...
# Deferred / Pending — Unification + RL Training Architecture (2026-05-11)

This section captures analysis and design proposals that have NOT yet been implemented. Driven by the LCCA 500-ep completion + cross-daughter reward audit + discussion of how to train RL on top of the current heuristic infrastructure.

## 31. Cross-Daughter 500-ep Snapshot (post-§29/§30, pre-unification)

The §30 result table reported preliminary numbers; the full 500-ep runs for all four daughters (post-grace patch) are now in:

| daughter | run dir | success | reach % | success mean R | dominant failure |
|---|---|---|---|---|---|
| RVA  | `2026-05-11_202046_env5_rl8_RVA_v1_500ep`  | 3/484 (0.6%)  | 33%   | +3.87 | trunk-wedge (47%) |
| RCCA | `2026-05-10_033729_env5_rl8_RCCA_v1_500ep` | 14/484 (2.9%) | 33.7% | +4.52 | trunk-wedge (55%) |
| LCCA | `2026-05-11_222213_env5_rl8_LCCA_v1_500ep` | 7/484 (1.4%)  | 91.7% | +1.09 | wbt at LCCA daughter (70 episodes / 14%) — geometry delivers wire into LCCA easily, but can't drive deep |
| LVA  | `2026-05-11_042358_env5_rl8_LVA_v2_500ep`  | 92/484 (19.0%)| 27%   | −3.10 | trunk-wedge (50%); successes have negative-mean R due to (19) flicker penalty stream (pre-suppression-patch) |

Three distinct failure profiles:
- **RVA/RCCA**: trunk-top fails to deflect into bridge (11). Phase B at bif2 isn't pulling the wire across.
- **LCCA**: opposite problem — wire enters LCCA daughter trivially (92%) but stalls at shallow depth; 70 wbt fires inside the daughter indicate centerline drift, not deflection.
- **LVA**: rewards artifact (post-suppression-patch will fix). 92 of 484 reach success at LVA — by far the best raw success rate.

Suppression patch (3-step grace + `_heur_suppress_wrong_branch` flag) landed in env5 AFTER the LVA v2 500-ep finished, so the −3.10 success-mean is the pre-fix number. Expected post-fix: ~+2 to +3 mean (worst-case successes lose 20-25 of penalty stream when (19) bursts no longer fire entry/step penalty).

## 32. Discrepancy Audit — Heuristic vs Reward vs Observation

Comprehensive audit (general-purpose agent, read all 4 daughter heuristics + env5 + 3 reward components + LocalGuidance + pathcontext + agent.py + action_curriculum.py + heuristic_controller.py) found 14 mismatches across three layers: 5 critical, 6 moderate, 3 cosmetic.

### Critical findings

**A1 — LVA's `_heur_suppress_wrong_branch` silences penalties but doesn't generate progress reward.** [heuristic_policy_lva.py:357-369](training _scripts/util/heuristic_policy_lva.py#L357-L369) sets the flag when `s > lva_jn_arc AND not on_correct_path`. [env5.py:538-548](training _scripts/util/env5.py#L538-L548) honors it for the penalty branch. But [arclengthprogress.py:99-109](eve/eve/reward/arclengthprogress.py#L99-L109) projects onto the planned-path polyline only — when the wire is in branch (19), `proj.s` pins at the LVA endpoint, so progress reward is 0. The demonstrator drives forward unrewarded; this is the LVA negative-cumulative bug at its root.

**A2 — +1 daughter-entry reward gate dead-zone.** [env5.py:566-573](training _scripts/util/env5.py#L566-L573) requires `arc_past_last_daughter ≥ 10.0`. Heuristic Phase C triggers at `s ≥ jn_arc − 5` ([heuristic_policy_rva.py:296-301](training _scripts/util/heuristic_policy_rva.py#L296-L301)). 10-mm dead zone where heuristic is doing the right thing with no reward signal.

**B1 — Agent observation has no absolute arclength, no junction-arc, no count-of-bifurcations-passed.** [localguidance.py:179-180,318-326](eve/eve/observation/localguidance.py#L179-L180) exposes only `d_rem_norm = (total − s)/total` (feature 0) and clipped `dist_to_bifurc` (feature 6). Two episodes with the same `d_rem_norm = 0.4` can be in completely different phases since the LVA path is shorter than the RVA path. Agent cannot reproduce "fire Phase C at `s ≥ rva_jn_arc − 5`."

**B2 — Heuristic computes J-tip bend from 3D tracking points; agent gets 2D.** [heuristic_policy_rva.py:158-188](training _scripts/util/heuristic_policy_rva.py#L158-L188) uses `bend = p0 + p2 − 2*p1` with full vessel-CS coords. [env5.py:114](training _scripts/util/env5.py#L114) constructs `Tracking2D(n_points=5)` — image-plane projection, y-axis dropped. Agent cannot reproduce the closed-loop rotation calculation from its observation.

**B3 — Heuristic phase state isn't in the observation.** `_heur_rva_phase`, `_heur_*_jn_arc`, `_heur_suppress_wrong_branch` are stashed on `base_env` for STEP-log diagnostics only ([env5.py:677,864](training _scripts/util/env5.py#L677)). The single best summary of state at a bifurcation is invisible to the agent.

### Moderate findings

- **C1**: [heuristic_policy_lcca.py:377-393](training _scripts/util/heuristic_policy_lcca.py#L377-L393) hard-codes Phase C parameters; ignores `_phase_c_variant`. Factorial grid (C0-C8) silently skips LCCA.
- **D1**: +1 reward uses `_path_daughter_arclengths` (real forks: ≥1 on-path branch AND ≥1 off-path branch); heuristic Phase-A milestone for LVA is the trunk-top `(2)→(0)` crossing which has no off-path branches on the LVA route → reward and heuristic disagree on what's a "meaningful junction event".
- **A4**: `is_on_correct_path()` applies the on-path-mask override ([pathcontext.py:742-775](eve/eve/util/pathcontext.py#L742-L775)); `arclengthprogress.get_projection()` does not — same wire state, two different verdicts.
- **B4**: LCCA Phase B uses 3D `target.coordinates3d`; agent's `Target2D` is the 2D image-plane projection — y-axis component dropped.
- **B5**: Agent sees tangent at current `s`; heuristic C2 default uses tangent at `s + 5`.
- **D2**: `COMMIT_HYSTERESIS_MM = 10.0` is fixed across all junctions; short bridges like (19) (~13 mm) may overshoot.

### Cosmetic findings

C2 (RCCA/LVA docstrings copy-pasted from RVA, say "RVA daughter"), C4 (LCCA/RCCA/LVA all write to `_heur_rva_phase` to fool the STEP-log writer — diagnostic-only), E2 (`action_curriculum.py` Stage 2 multiplies `cath_trans` by 0.1 even for heuristic-mode demos — distorts seeding distribution).

## 33. Proposed Unification — `path_extension_set`

**Goal.** Make heuristic, reward, and env5 wrong-branch logic agree on what counts as "wire making valid progress along the route".

**Concept.** Alongside the strict `path_branch_idx_set` (branches in the planned-path polyline), define a per-target `path_extension_set`: branches that are geometrically/functionally the continuation of the planned path but aren't in the planner's polyline. Built at episode reset from a small per-target table:

| target | extension set | reason |
|---|---|---|
| LVA   | `{(19)}` | branch (19) is the deep continuation of LVA (same physical vessel, two centerline annotations) |
| RVA   | `{}`     | no equivalent |
| RCCA  | `{}`     | no equivalent |
| LCCA  | `{}`     | possibly add later if LCCA depth pathology turns out to be a similar centerline-annotation gap |

### Implementation surface

| File | Change |
|---|---|
| [eve/eve/util/pathcontext.py](eve/eve/util/pathcontext.py) | Add `path_extension_set` field + per-target lookup table, built at reset. Union into `is_on_correct_path()`: returns True if `_current_branch_idx ∈ (path_branch_idx_set ∪ path_extension_set)`. Add helper `get_progress_polyline_and_cumlen()` → returns either planned-path polyline (default) OR the wire's currently-occupied extension-branch polyline (when in extension). |
| [eve/eve/reward/arclengthprogress.py](eve/eve/reward/arclengthprogress.py) | Replace direct `_path_context.get_projection()` with the helper. When wire is in the extension, progress is computed along that branch's centerline so forward motion earns +0.01/mm. |
| [training _scripts/util/env5.py](training _scripts/util/env5.py) | Delete `_heur_suppress_wrong_branch` reads at both [env5.py:538](training _scripts/util/env5.py#L538) (penalty block) and [env5.py:864](training _scripts/util/env5.py#L864) (`_resolve_termination_reason`). Both checks already key off `is_on_correct_path()` which now naturally returns True when wire is in the extension. |
| [training _scripts/util/heuristic_policy_lva.py](training _scripts/util/heuristic_policy_lva.py) | Delete the suppression-flag write and the off-path override branch at [lines 351-369](training _scripts/util/heuristic_policy_lva.py#L351-L369). The `_detect_phase` `lva_daughter` regime now fires naturally via `is_on_correct_path() == True` when wire is in (19). |

### What this gets us

- Heuristic stops needing the suppression hack — the wire is genuinely on-path by the new definition.
- LVA-target wires advancing through (19) earn +0.01/mm of forward motion along (19)'s centerline, not 0. A wire that walks 30 mm of (19) earns +0.3 of shaping instead of accruing penalty stream.
- Wedged-off-path wires (different code, in some sister branch not on the route) still get no progress and still get penalized — extension set is target-specific.
...
### Estimated reward distribution impact (LVA targets)

The 92 LVA SUCCESS episodes currently have mean R = −3.10, median R = +2.26, with 7 outliers at R < −20. The −20 outliers spent 200-400 steps in (19) flicker accumulating ~25 of net penalty each. Post-unification, those re-center to roughly 0 to +3. Expected post-fix LVA SUCCESS mean: roughly +2 to +2.5. Some MAX-STEPS-but-near-target episodes may also flip to SUCCESS, since `wrong_branch_timeout` won't fire on (19) flicker that exceeds 50 consecutive off-path steps in pathological cases.

## 34. Additional Discrepancies Worth Fixing (beyond unification)

The unification in §33 fixes the heuristic/reward/wrong-branch triad (A1, A4, D1). Three further fixes are independent and orthogonal:

### 34a. Observation enrichment — fix B1 + B2 + B3

The single shared `LocalGuidance` observation doesn't expose the state variables the heuristic routes through. Without this, even imitation learning will plateau because the policy class isn't expressive enough given the input.

Proposed feature additions to [localguidance.py](eve/eve/observation/localguidance.py):

- `proj.s / total_length` (already there as feature 0, keep).
- `arc_to_next_daughter / 100` (mm, clipped to [0, 1]) — discriminates "we're approaching a junction" from "we're mid-vessel".
- `arc_past_last_daughter / 100` (mm, clipped to [0, 1]) — heuristic Phase-C trigger uses this exact quantity.
- 4-dim one-hot phase indicator (Phase A / B / C / default) — read from `_heur_*_phase` if present, else zero vector. Drops the need for the agent to discover phase boundaries from raw arclength.
- 2-dim `bend_hat` projected to image plane — replaces the geometric reasoning in `_compute_rotation_to_target()` with a single drop-in feature. Lets the agent reproduce the heuristic's closed-loop steering signal.

Cost: ~30 LOC. Side-effect: changes observation dim from N to N+8, requires retraining any existing RL checkpoints from scratch.

### 34b. Reward trigger alignment — fix A2

[env5.py:565](training _scripts/util/env5.py#L565) sets `arc_past_last_daughter >= 10` as the +1 reward trigger. Move it to either `arc_past_last_daughter >= 5` (matches heuristic `rva_cavity` end) or `s >= jn_arc − 5` (matches heuristic Phase-C onset). ~5 LOC. The current 10-mm gap means the demonstrator does steering that the reward function ignores.

### 34c. LCCA variant dispatch — fix C1

Copy RVA's `PHASE_C_VARIANTS` table into [heuristic_policy_lcca.py](training _scripts/util/heuristic_policy_lcca.py) and dispatch the same way as RVA/RCCA/LVA. ~50 LOC. Without this the factorial grid test (§28) silently doesn't cover LCCA.

## 35. RL Training Architecture — Per-daughter vs Shared

The four-daughter empirical results (§31) and the heuristic-policy differences raise a strategic question: train one RL policy with daughter identity in observation, or four separate per-daughter policies?

### Empirical asymmetry across daughters

The four daughters require **substantively different Phase B/C steering**:

| daughter | Phase B steering | Phase C steering |
|---|---|---|
| RVA  | `dynamic_tangent` at `s+5`, original sign, RVA-aimed | same |
| RCCA | exact mirror of RVA | exact mirror of RVA |
| LCCA | position-vector to LCCA waypoint at `lcca_jn+25 mm` | `dynamic_tangent` at `s+5` |
| LVA  | doesn't exist (let trunk-ascent physics deliver wire to (18)) | `dynamic_tangent` at `s+5` |

Heuristic success rates also span 30× (LVA 19% vs RVA 0.6%). Without intervention, RL training will see ~19/24 of successful demos from LVA. Naïve SAC training pulls the policy toward "behave like LVA-agent" for every target.

### Shared model

**Pros.** (a) ~50-65% of every episode is trunk(2) ascent, identical correct-action regardless of target — shared model accumulates 4× the data for this dominant sub-task. (b) Cross-daughter generalization: bif2 wedge-recovery is the same physical event for all targets; one model gets 4× the negative examples. (c) Per-target buffer balancing + curriculum tools (HER, PER, hindsight-relabeling) available cleanly. (d) Single deployment artifact; new target locations are config changes. (e) Robust to deployment-time distribution shift.

**Cons.** (a) Severe class imbalance: heuristic seeding sees 30× spread in success rates → naïve gradient is LVA-dominated. (b) The four Phase-B/C policies are genuinely distinct; shared network must learn 4 conditional sub-policies given `target_state` as discriminator. Capacity probably OK at standard SAC widths (256-512) but the OPTIMIZATION (gradient averaging in batches) biases toward majority target. (c) Reward magnitude varies 3-4× across daughters (LVA path ≈ 140 mm, LCCA ≈ 60 mm). Critic learns wider Q range. (d) Per-daughter regression hard to detect without per-target metric logging. (e) Training-time instability in one daughter can degrade inference on all.

### Per-daughter models

**Pros.** (a) No class imbalance — each model sees only its target's experiences. (b) Dedicated capacity per steering strategy. (c) Per-daughter hyperparam tuning. (d) Trivially parallelizable (4 GPUs = 4× wall-clock). (e) Debugging is local. (f) Heuristic seeding infrastructure already exists per-daughter (4 runners + 4 launch scripts).

**Cons.** (a) 4× total compute. (b) Trunk-traversal sample inefficiency — each model rediscovers trunk push from 1× data. (c) 4× maintenance: training runs, hyperparam configs, deployments, checkpoints, evaluation pipelines. (d) No transfer to new targets. (e) Slightly inconsistent action distributions for indistinguishable trunk states across the four policies.

### Hybrid — shared backbone + per-daughter heads

Worth considering as middle ground: one shared feature extractor over `tracking`, `last_action`, `guidance`; 4 small policy heads routed by `target_state`. Critic similar. Shares trunk-traversal feature learning; per-daughter head specializes Phase B/C. Implementation cost: modify eve_rl actor/critic structures; ~150 LOC.

### Recommendation for the first RL training cycle

**Start per-daughter.** Three reasons:

1. **Sample-efficiency from heuristic seeding dominates at this stage.** Each per-daughter heuristic produces clean demonstrations. Per-daughter agent ingests pure on-policy demos. Shared agent ingests 19× more LVA demos than RVA demos and the seeding asymmetry compounds the gradient bias.
2. **Per-daughter baselines for comparison.** Without 4 baselines, a future "70% overall" shared model can't be diagnosed — it might be 100% LVA + 40% on the others.
3. **Trunk-traversal share is overstated.** Each daughter's heuristic injects ~250-400 trunk-traversal steps × 500 episodes ≈ 150k steps per model. Plenty for the trunk sub-policy; marginal benefit of 4× is small past the data-density elbow.

After per-daughter models converge or plateau, run the shared-model experiment with per-daughter buffers as initialization (multi-task fine-tuning). That experiment answers the interesting research question — "does shared representation help generalize?" — with per-daughter ceilings in hand.

## 36. HER / PER Analysis for the Shared-Model Path

If the shared-model path is taken, PER and HER address Con (a) — class imbalance — but not Con (b) — capacity.

### PER — strong yes for class imbalance

A successful RVA terminal transition has reward +3 against a critic-predicted value near 0 (critic has barely seen RVA successes). TD-error is large; PER samples it ~5-10× more often than its 0.6% natural frequency. Effective gradient contribution from RVA successes scales with learning signal, not episode frequency. Directly attacks the "policy behaves like LVA-agent for every target" failure mode.

Combined with post-unification reward (no spurious high-TD-error spikes from (19) flicker), PER amplifies genuine learning signal, not artifacts.

### HER — partial yes, with structural caveat

HER converts "failed to reach goal G" trajectories into "reached achieved-state G'". Works for near-miss failures, not for structurally-wrong-path failures. The yield depends on failure-mode mix per daughter:

| daughter | near-miss-in-daughter % | bif2-wedge % | HER yield |
|---|---|---|---|
| LVA  | ~16% | 50% | medium |
| LCCA | **~87%** | ~5% | **huge** — almost every failure is HER-relabelable |
| RCCA | ~17% | 55% | medium |
| RVA  | ~16% | 47% | medium |

HER would dramatically benefit LCCA (its 1.4% success becomes effectively 80%+ after relabeling). Asymmetry doesn't equalize class imbalance directly but combined with PER (which still upweights rare bif2-wedge experiences by TD-error) the net is roughly balanced.

Critical implementation constraint: only relabel to achieved states that ARE on a daughter centerline. Otherwise the policy learns "drive to wedge cluster". Need to check if `eve_rl/replaybuffer/vanillaepisode.py` supports goal-relabeling; if not, ~100 LOC wrapper.

### Cheaper alternative — per-target replay buffers

Maintain 4 replay buffers (LCCA/LVA/RCCA/RVA), sample equally from each per gradient step (16 transitions = 4 from each buffer). Guaranteed cross-target balance. No PER β tuning, no HER relabeling logic. Doesn't upweight high-TD-error transitions WITHIN a target (PER does); just balances ACROSS targets.

### Recommended stack if shared model is attempted

1. Per-target replay buffer balancing (cross-daughter balance, trivial).
2. PER within each buffer (intra-daughter success-vs-failure balance).
3. HER with daughter-centerline-restricted relabeling (multiplies near-miss episodes into synthetic successes — especially valuable for LCCA's 91% reach).
4. Augment `target_state` with 4-dim one-hot daughter indicator alongside 2D coords, so the network doesn't have to discover discrete target identity from continuous coordinates.

Stack neutralizes most of Con (a) and side-steps Con (b) by making daughter identity highly observable.

## 37. Pending TODO — execution order

Independent of which RL architecture path is chosen, the unification work in §33 is a prerequisite (heuristic/reward consistency directly affects training signal quality). Suggested execution order:

1. **Unification (§33)** — 4 files, ~150 LOC total. Verify with LVA 500-ep re-run.
2. **Reward trigger alignment (§34b)** — 5 LOC.
3. **LCCA variant dispatch (§34c)** — 50 LOC.
