#!/bin/bash
# Gen-4 — RCCA P2 teacher v1c (RL_IMPROV_18, branch rl_improv_18_p2).
#
# v1b RESULT: residual-on-heuristic privileged-actor teacher plateaued at
# ~61% explore / ~62% eval-avg (STABLE, no collapse; H0 ~37-47% band, v2
# ref 49%-then-collapse). Cap mechanism: two failure channels — permanent
# junction-capture stalls + smooth wrong-turns — and the SOFT recovery
# skill never developed (flat 2-6%); success-linked crunch engagement
# peaked in the eval3 era (81.6% crest) and faded.
#
# v1c = v1b + ONE new lever (single-variable experiment):
#   --crunchpass_fraction 0.2   CRUNCHPASS replay lane — 20% of every
#       batch drawn from transitions with the dual-device crunch
#       SIGNATURE (both device translations engaged >0.25 normalized AND
#       local_radius <= 0.175, i.e. the deep-daughter narrow band) AND
#       whose episode SUCCEEDED (post rail-filter). Verified on v1b's
#       retained chunks (saved/p2a_deep_dive/CRUNCH_POOL_STRATEGY.md):
#       the raw signature alone marks GRINDING (failures carry 4-8x more
#       signature steps in every era) — success-conditioning selects the
#       productive crunch passages (machine-2 v3a classifier insight; the
#       v3b stuck-harvest lesson: truly-stuck states are unrecoverable
#       knots, so we resample the states where crunch transit WORKS, not
#       where it already failed).
#   --balanced_fraction 0.3 -> 0.2   crunchpass is a sharpened subset of
#       the clean lane; composition 0.2 clean + 0.2 crunchpass + 0.6
#       general keeps total success-amplification comparable to v1b.
#   --replay_buffer_size 2M -> 1.5M   OOM mitigation: v1b died at day 4
#       (mem crept past the 23.5GiB host cap -> kernel killed a worker ->
#       futex deadlock -> watchdog exit 42, AS DESIGNED). ~25% smaller
#       steady-state footprint; 1.5M still exceeds the useful window.
#
# Unchanged from v1b: residual heuristic scale 1.0, heur_action_obs (125),
# privileged actor, sac + LayerNorm critics + no entropy backup, alpha
# rails [-5,0], target_entropy 1.0, heatup 20k @ 0.3 band, pretrain 0 +
# baseline eval (H0), PER, 16 procedural workers, R1-R3 sync fixes +
# NET_SYNC alarm + stall-gated deadline + watchdog.
#
# Decisive checks:
#   * [v1c] crunchpass lane ON line at startup; lane fills from first
#     heatup successes (run starts at heuristic level — no cold start).
#   * NET_SYNC ok lines advancing with CHANGING fingerprints (main.log).
#   * Explore success vs v1b's ~61% plateau — the single-variable readout.
#   * Soft-recovery rate (windowed tracker) vs v1b's flat 2-6% — the lane
#     exists to move exactly this.
#   * Eval band vs v1b's 51-81 (use multi-eval averages; single evals
#     carry +-10-15pp start-rotation noise).
# Monitor: docker logs -f rcca_p2_teacher_v1c | Stop: docker stop -t 60 rcca_p2_teacher_v1c
set -e
export MSYS_NO_PATHCONV=1

docker rm rcca_p2_teacher_v1c 2>/dev/null || true

docker run --name rcca_p2_teacher_v1c --gpus all --shm-size=30g --init -d \
