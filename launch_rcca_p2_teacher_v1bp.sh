#!/bin/bash
# ============================================================================
# v1bp = BYTE-IDENTICAL v1b + ONLY the v3c reward pair (single-variable):
#   --progress_tip_mode avg --avg_gw_weight 0.5 --cath_slack_coef 0.5
# NO crunchpass lane (parked for the next experiment), balanced stays 0.3,
# replay stays 2M (identical science; accepted risk: v1b's day-4 host-mem
# OOM horizon — the eval1-3 target window resolves within ~24h, well before).
# Baselines to beat (v1b): H0 band 37-47% (H0 SUCCESS must land IN this band
# — the heuristic is reward-blind; outside = port bug, STOP. H0 REWARD may
# differ slightly ~0.9x progress pay), eval1 68.4 / eval-avg 62 / explore
# plateau 61. Era-verified premise: v1b's eval2-3 crest ran at 6.2%
# cath-lead vs 58-62% in its decline — the pair pays the crest behavior and
# taxes the drift. WATCH (handoff §7): q1_mean sustained < -2.5 = critic
# diverging. Startup must echo BOTH [v3c] lines.
# ============================================================================
...
# Monitor:  docker logs -f rcca_p2_teacher_v1bp  |  Stop: docker stop -t 60 rcca_p2_teacher_v1bp
set -e
export MSYS_NO_PATHCONV=1

docker rm rcca_p2_teacher_v1bp 2>/dev/null || true

docker run --name rcca_p2_teacher_v1bp --gpus all --shm-size=30g --init -d \
