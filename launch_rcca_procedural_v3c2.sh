#!/bin/bash
# RL_IMPROV_16 (v3c) — rcca_procedural_v3c2: v3c run 1 + the SLACK-TAIL FIX.
#
# v3c run 1 result (eval1 @252k explore): Quality 62.2% (61/98) vs v3a's
# all-time best 52% over 15 evals/3.5M steps; explore success 38.7%->57%;
# successes deploy 76mm of guidewire (v3a: 0.2-9.4mm). The reward pair works.
#
# WHAT CHANGED (code constant only, NOT a flag): the catheter-slack potential
# tail in util/buckle_reward.py. The old design clipped the INPUT at 150mm so
# phi_c bottomed at -1.0 and the gradient went to ZERO beyond it. Run-1's 37
# remaining failures exploited exactly that dead zone: both devices railed to
# their ~898mm insertion caps with ~708mm of catheter slack, 4.7x past the cap,
# with no marginal deterrent. The fix keeps the SLOPE identical (1.0 potential
# per 150mm excess) and extends the linear range past the physical device
# limit, so a 708mm coil now costs -2.31 return instead of -0.50.
#
# SEED UNCHANGED — reuse saved/rcca_proc_heatup_v3c/seed.npz. Verified safe:
# across all 282,657 harvest steps / 480 episodes the max catheter slack ever
# reached is 128.9mm, so the old 150mm cap NEVER binds anywhere in the seed
# (0/480 episodes end above it, 0.0000% of steps exceed it). The cap change is
# therefore a provable no-op on seed rewards and the reward-version guard
# stamps stay valid. NOTE: the guard covers the flagged coefficients, not this
# code constant — run provenance for the tail lives in this header + git.
#
# TRAINER LEVERS still default-OFF (reserved for a later run).
# GATES: does Quality hold/climb past 62.2%, and do failures stop railing both
# devices to the cap (watch median final cath_slack: run 1 = 708mm)?
set -e
...
docker rm rcca_procedural_v3c2 2>/dev/null || true

docker run --name rcca_procedural_v3c2 --gpus all --shm-size=24g --init -d \
  ...
  -e STUCK_CHECKPOINT_DIR=/opt/eve_training/results/rcca_v3c2_stuck \
...
# Monitor:  docker logs -f rcca_procedural_v3c2   |   Stop:  docker stop rcca_procedural_v3c2
