#!/bin/bash
# LVA-target heuristic v2 — Phase A (+z degenerate, gw_trans=10) for
# trunk → (18) bridge crossing, Phase C daughter for inside-LVA advance.
# Plus LVA-specific override: Phase C keeps firing even when off-path
# (bypasses default retract handler in the LCCA-LVA convergence flicker
# region). 500 episodes for parity with RVA/RCCA v2cfg-500-inst runs.
set -e
export MSYS_NO_PATHCONV=1

docker rm env5_rl8_LVA_v2_500ep 2>/dev/null || true

docker run --name env5_rl8_LVA_v2_500ep --gpus all --shm-size=24g --init -d \
