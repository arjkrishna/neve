#!/bin/bash
# Eval-only (Plan v10 follow-up): run the eval-#1 policy (checkpoint250531.everl
# = the 32.67% deterministic policy) on the SAME 98 RCCA EVAL_SEEDS, with EVERY
# episode SOFA-restored from the single best start state pid19116 (~82% in
# eval #1). Isolates the per-state ceiling of that exact policy.
#
# Routes through DualDeviceNav_train.py --eval_only_checkpoint so the run
# produces the STANDARD log/snapshot structure: a fresh timestamped run dir
# under saved/eve_paper/.../mesh_ben/<ts>_rcca_eval_pid19116_v1/ with
# logs_subprocesses/worker_*.log, diagnostics/snapshots/eval/RCCA/{success,
# wrong_branch_timeout, wire_fold_stall, max_steps}/, main.log, etc.
#
# Run AFTER stopping rcca_awac_v3 (frees cuda:0).
...
docker rm rcca_eval_pid19116 2>/dev/null || true

docker run --name rcca_eval_pid19116 --gpus all --shm-size=24g --init -d \
  ...
  python3 /opt/eve_training/training_scripts/DualDeviceNav_train.py \
    ...
    --checkpoint_dir /opt/eve_training/results/rcca_best_state_pid19116 \
    --eval_only_checkpoint ${EVAL_CKPT} \
    -nw 16 -d cuda:0
# After it finishes:
#   docker logs -f rcca_eval_pid19116
#   results dir: saved/eve_paper/neurovascular/full/mesh_ben/<ts>_rcca_eval_pid19116_v1/
...
#     - main.log  ("evaluation: ..." + per-episode results line)
#     - checkpoints/  (an eval-only checkpoint will be written; harmless)
